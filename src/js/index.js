...
/*1. r funções async dentro do Promise.all(), afinal elas retornam uma promise. Porém, como o
próprio Promise.all() retorna uma promise também podemos utilizar o await com ele. No código abaixo vemos 
que a função .map() está retornando um array de promises pendentes:
...
/*função que renderiza esses dados na tela*/
...
/* o uso do fetch() para requisições GET como vimos na 
aula anterior. No arquivo index.js iremos criar 
uma função para obter os artigos salvos no db.json e
outra para renderizá-los na tela, além de chamar a função 
fetchArticles() assim que o documento estiver carregado*/
...
/*Repare que se adicionarmos uma action e um method ao
nosso form ele já consegue fazer requisições para o json-server,
criando um novo artigo, e como a página é atualizada o novo artigo
já aparece na tela. No entando, o que queremos é fazer as requisições
POST através do javascript sem atualizar a página. Para isso podemos 
usar o fetch() novamente, dessa vez com algumas opções a mais*/
...
/*Agora vamos treinar um pouco mais o uso do fetch() para requisições GET como vimos na aula anterior.
No arquivo index.js iremos criar uma função para obter os artigos salvos no db.json 
e outra para renderizá-los na tela, além de chamar a função fetchArticles() assim que o documento estiver carregado;*/
...
/*Repare que se adicionarmos uma action e um method ao nosso form ele já consegue fazer
requisições para o json-server, criando um novo artigo, e como a página é atualizada o novo
artigo já aparece na tela. No entando, o que queremos é fazer
as requisições POST através do javascript sem atualizar a página. Para isso podemos 
usar o fetch() novamente, dessa vez com algumas opções a mais:*/
...
/*fazendo uso do await e dos blocos try e catch:*/
...
/* funções async, porém apenas com o resolve. Mas as funções async também permitem 
rejeitar uma promise. Podemos fazer isso através do objeto global Promise e seu método .reject()*/
